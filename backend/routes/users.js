...
router.get('/metrics/:userId',checkAuth, async (req, res) => {
    const userId = req.params.userId;

    try {
      // Find the user by ID and only return the metrics field
      const user = await User.findById(userId).select('fitnessMetrics');
  
      if (!user) {
        return res.status(404).json({ message: 'User not found' });
      }
  
      res.status(200).json({ metrics: user.fitnessMetrics });
    } catch (error) {
